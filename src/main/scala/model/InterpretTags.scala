...
import model.AuxTypes.{MessageId, PlainUser}
...
val Login = Extract[UserNoticeTags, PlainUser]("login", x => Some(PlainUser(x)))

object Type:
  @targetName("unapplyUserNoticeTags")
  def unapply(tags: UserNoticeTags): Option[(UserNoticeType, UserNoticeTags)] =
    Extract[UserNoticeTags, UserNoticeType]("msg-id", strToUserNoticeType).unapply(tags)

  @targetName("unapplyNoticeTags")
  def unapply(tags: NoticeTags): Option[(NoticeType, NoticeTags)] =
    Extract[NoticeTags, NoticeType]("msg-id", strToNoticeType).unapply(tags)

extension (privmsgTags: PrivmsgTags)
  def getBits: Option[Int]     = Bits.unapply(privmsgTags).map(_._1)
  def getId: Option[MessageId] = Id.unapply(privmsgTags).map(_._1)

extension (userNoticeTags: UserNoticeTags)
  @targetName("userNoticeTagsGetType")
  def getType: Option[UserNoticeType] = Type.unapply(userNoticeTags).map(_._1)
  def getLogin: Option[PlainUser]     = Login.unapply(userNoticeTags).map(_._1)

extension (noticeTags: NoticeTags)
  @targetName("noticeTagsGetType")
  def getType: Option[NoticeType] = Type.unapply(noticeTags).map(_._1)
...
val strToNoticeType: String => Option[NoticeType] = {
